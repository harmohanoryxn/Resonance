...
namespace Cloudmaster.WCS.Department.Processing.Feeds
{
	public class GetIguanaFeedsProcessor //: ProcessorBaseClass
	{
		public void ExecuteInBackround()
		{
			...
			var backGround = System.Threading.Tasks.Task.Factory.StartNew(() => OnDoWork(arguements));
			//backGround.ContinueWith(t => RaiseWorkEndedEvent(), TaskScheduler.FromCurrentSynchronizationContext());

		}

		private  void OnDoWork(GetIguanaFeedsProcessorArguements e)
		{
			ProcessorArguements arguements = (ProcessorArguements)e;
...
//			e.Result = results;
			...
			Application.Current.Dispatcher.Invoke(DispatcherPriority.Normal, (Action<ProcessingResults>)OnRunWorkerCompleted, results);
       
		}

		private void OnRunWorkerCompleted(ProcessingResults e)
		{
