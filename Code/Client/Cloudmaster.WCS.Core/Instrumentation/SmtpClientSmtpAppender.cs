...
// Robbed directly from http://weblogs.asp.net/tgraham/archive/2007/04/20/a-log4net-appender-that-uses-smtpclient.aspx
    ...
    public class SmtpClientSmtpAppender : SmtpAppender
    {
        override protected void SendBuffer(LoggingEvent[] events)
        {
            try
            {
                ...
                MailMessage mail = new MailMessage(From, To, string.Concat("[", Environment.MachineName, "] ", Subject), messageText);
                client.Send(mail);
            }
            catch (Exception e)
            {
