...
{
    public class ExcelFormExporter
    {
        public void Export(string filePath, FormInstance formToExport, bool includeActionLinks)
        {
            using(SpreadsheetDocument package = SpreadsheetDocument.Create(filePath, SpreadsheetDocumentType.Workbook))
            {
                CreateParts(package, formToExport, includeActionLinks);
            }
        }

        // Adds child parts and generates content of the specified part.
        private void CreateParts(SpreadsheetDocument document, FormInstance formToExport, bool includeActionLinks)
        {
            ExtendedFilePropertiesPart extendedFilePropertiesPart1 = document.AddNewPart<ExtendedFilePropertiesPart>("rId3");
            GenerateExtendedFilePropertiesPart1Content(extendedFilePropertiesPart1);

            WorkbookPart workbookPart1 = document.AddWorkbookPart();
            GenerateWorkbookPart1Content(workbookPart1, formToExport);
            ...
            foreach (Section section in formToExport.Sections)
            {
                int indexOfSection = formToExport.Sections.IndexOf(section);

                string worksheetId = string.Format("rIdWS{0}", indexOfSection);
                string tableName = string.Format("Table{0}", indexOfSection);
                string tableId = string.Format("rIdTF{0}", indexOfSection);

                WorksheetPart worksheetPart1 = workbookPart1.AddNewPart<WorksheetPart>(worksheetId);

                int numberOfChecks = section.Checks.Count;

                int numberOfRows = numberOfChecks + 1;

                string tableDimensions = string.Format("A1:G{0}", numberOfRows);

                GenerateWorksheetPart1Content(worksheetPart1, section, tableDimensions, tableId, includeActionLinks);
        ...
        private void GenerateWorkbookPart1Content(WorkbookPart workbookPart1, FormInstance formToExport)
        {
            ...
            string worksheetName = string.Format("{0}", formToExport.Metadata.Name);
        ...
        private void GenerateWorkbookStylesPart1Content(WorkbookStylesPart workbookStylesPart1)
        {
            ...
            BackgroundColor backgroundColor1 = new BackgroundColor(){ Rgb = "FFC00000" };

            patternFill3.Append(backgroundColor1);

            fill3.Append(patternFill3);

            differentialFormat1.Append(font4);
            ...
            BackgroundColor backgroundColor2 = new BackgroundColor(){ Rgb = "FF92D050" };
            ...
            BackgroundColor backgroundColor10 = new BackgroundColor() { Theme = (UInt32Value)7U, Tint = -0.24994659260841701D };

            patternFill12.Append(backgroundColor10);

            fill12.Append(patternFill12);

            differentialFormat10.Append(font10);
        ...
        private void GenerateWorksheetPart1Content(WorksheetPart worksheetPart1, Section section, string tableDimensions, string tableId, bool includeActionLinks)
        {
            ...
            foreach (Check check in section.Checks)
            {
                ...
                Row row = GenerateRowForCheck(section, indexDisplayed, rowIndex, check, includeActionLinks);
            ...
            ConditionalFormatting conditionalFormatting1 = new ConditionalFormatting(){ SequenceOfReferences = new ListValue<StringValue>() { InnerText = "D1:D1048576" } };

            ConditionalFormattingRule conditionalFormattingRule1 = new ConditionalFormattingRule(){ Type = ConditionalFormatValues.CellIs, FormatId = (UInt32Value)1U, Priority = 2, Operator = ConditionalFormattingOperatorValues.Equal };
            Formula formula1 = new Formula();
            formula1.Text = "\"Pass\"";

            conditionalFormattingRule1.Append(formula1);

            ConditionalFormattingRule conditionalFormattingRule2 = new ConditionalFormattingRule(){ Type = ConditionalFormatValues.CellIs, FormatId = (UInt32Value)0U, Priority = 1, Operator = ConditionalFormattingOperatorValues.Equal };
            Formula formula2 = new Formula();
            formula2.Text = "\"Fail\"";

            conditionalFormattingRule2.Append(formula2);

            ConditionalFormattingRule conditionalFormattingRule3 = new ConditionalFormattingRule() { Type = ConditionalFormatValues.CellIs, FormatId = (UInt32Value)2U, Priority = 3, Operator = ConditionalFormattingOperatorValues.Equal };
            Formula formula3 = new Formula();
            formula3.Text = "\"Incomplete\"";
        ...
        private static Row GenerateRowForCheck(Section section, int index, int rowIndex, Check check, bool includeActionLinks)
        {
            ...
            cellValue8.Text = index.ToString();
            ...
            cellValue9.Text = string.Empty; // formToExport.Metadata.Location;
            ...
            cellValue10.Text = check.Description;
            ...
            if (check.Result.CompareTo("True") == 0)
            {
                cellValue11.Text = "Pass";
            }
            else if (check.Result.CompareTo("False") == 0)
            {
                cellValue11.Text = "Fail";
            }
            else
            {
                cellValue11.Text = check.Result;
            }
            ...
            cellValue12.Text = check.AssetNumber;
            ...
            cellValue14.Text = check.Comments;

            cell14.Append(cellValue14);

            Cell cell13 = new Cell() { CellReference = ExcelHelperClass.ConvertCellIndexToA1(7, rowIndex), StyleIndex = (UInt32Value)2U, DataType = CellValues.String };
            CellValue cellValue13 = new CellValue();

            cellValue13.Text = check.TaskId;
        ...
        private void GenerateTableDefinitionPart1Content(int Id, TableDefinitionPart tableDefinitionPart1, string tableName, string tableDimensions)
        {
            ...
            TableColumn tableColumn2 = new TableColumn(){ Id = (UInt32Value)2U, Name = "Location" };
            TableColumn tableColumn3 = new TableColumn(){ Id = (UInt32Value)3U, Name = "Description" };
            TableColumn tableColumn4 = new TableColumn(){ Id = (UInt32Value)5U, Name = "Result" };
            TableColumn tableColumn5 = new TableColumn(){ Id = (UInt32Value)4U, Name = "Asset Number" };
            TableColumn tableColumn6 = new TableColumn(){ Id = (UInt32Value)6U, Name = "Comments" };
            TableColumn tableColumn7 = new TableColumn(){ Id = (UInt32Value)7U, Name = "Work Order Number" };
