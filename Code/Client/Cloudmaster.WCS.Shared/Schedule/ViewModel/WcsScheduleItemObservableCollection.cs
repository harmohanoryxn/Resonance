...
namespace WCS.Shared.Schedule
{
	...
	public abstract class WcsScheduleItemObservableCollection3<TVM, VM, V> : ObservableCollection<VM>
		where VM : class, ISynchroniseable<VM>, IIdentifable
		where V : IIdentifable
	{
		private Collection<VM> _unfilteredCollection;
		private Func<V, TVM, VM> _transformFunction;
		...
		public WcsScheduleItemObservableCollection3(Func<V, TVM, VM> transformFunction)
		{
		...
		public void Synchronise(IList<V> itemList, TVM patient)
		{
			...
			lock (SyncRoot)
			{
				...
				var newItems = itemList.Where(i => additionIds.Contains(i.Id)).Select(o => _transformFunction(o, patient)).ToList();
				var differentItems = itemList.Where(i => differenceIds.Contains(i.Id)).Select(o => _transformFunction(o, patient)).ToList();
		...
		protected void Synchronise(V item, TVM topViewModel)
		{
			lock (SyncRoot)
			{
				var diffItem = _transformFunction(item, topViewModel);
		...
		public void Filter()
		{
			lock (SyncRoot)
			{
			    ...
			    {
                    Application.Current.Dispatcher.InvokeIfRequired(DoFilter);
			    }
		        else
			    {
                    DoFilter();
